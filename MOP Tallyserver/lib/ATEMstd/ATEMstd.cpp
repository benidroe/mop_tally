 ...
 */
char *  ATEMstd::getInputShortName(uint16_t videoSource) {
...
long ATEMstd::getAudioMixerLevelsSourceRight() {
	return atemAudioMixerLevelsSourceRight;
		...
		void ATEMstd::_parseGetCommands(const char *cmdStr)	{
			...
			uint16_t index,audioSource,sources,videoSource;
			long temp;
            ...
            uint32_t ATEMstd::getTallyByFlag(int flag) {


				uint32_t result = 0;
				// Loop over an array and print each element
					for (int i = 0; i < 21; i++) {
						if(atemTallyByIndexTallyFlags[i] == flag){
